...
public class LandingActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.buttonBikes) {
            intent.putExtra(EXTRA_VEHICLE_TYPE, Common.TYPES[4]);
        } else if (v.getId() == R.id.buttonTrucks) {
            intent.putExtra(EXTRA_VEHICLE_TYPE, Common.TYPES[2]);
        } else if (v.getId() == R.id.buttonTractors) {
            intent.putExtra(EXTRA_VEHICLE_TYPE, Common.TYPES[1]);
        } else if (v.getId() == R.id.buttonTempos) {
            intent.putExtra(EXTRA_VEHICLE_TYPE, Common.TYPES[3]);
        } else {
            intent.putExtra(EXTRA_VEHICLE_TYPE, Common.TYPES[0]);
        }
